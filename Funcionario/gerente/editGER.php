...
$nome = $cidade = $bairro = $rua = $residencia = $login = $senha = '';
$codigo = '';

// Verifica se o código do entregador foi fornecido
if (!empty($_GET['CdGerente'])) {
    $codigo = intval($_GET['CdGerente']); // Assegura que o código é um inteiro

    // Seleciona os dados do entregador
    $sqlSelect = "SELECT * FROM TbGerente WHERE CdGerente=$codigo";
    $result = $conexao->query($sqlSelect);

    if ($result->num_rows > 0) {
        // Preenche as variáveis com os dados do entregador
        $user_data = $result->fetch_assoc();
        $nome = $user_data['NmGerente'];
...
if (isset($_POST['submit'])) {
   ...
   // Preparar a consulta para atualização
    $sqlUpdate = "UPDATE TbGerente SET
    NmGerente=?,
    NmCidade=?,
    NmBairro=?,
    NmRua=?,
    ...
    $stmt->bind_param('sssssssi', $nome, $cidade, $bairro, $rua, $residencia, $login, $senha, $codigo);
